...
test_landing_time <- function(phase, rwys = rwys_egll){
  ...
  with_ldg_time <-  arrs |>
   ...
   # dplyr::mutate(study_trj = map(.x = study_trj, .f = ~ interpolate_lin_4D(.x, .source = "OSN"))) |> 
    dplyr::mutate(ALDT = map(.x = study_trj, .f = ~ filter(.x, MST == "ALDT") |> pull(TIME))) |> tidyr::unnest(ALDT)
